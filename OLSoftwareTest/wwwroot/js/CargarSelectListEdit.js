    ...
    $.ajax({
        ...
        success: function (data) {
            if (data != null && data != undefined && data.length > 0) {
                ...
                for (var i = 0; i < data.length; i++) {
                    console.log(data[i])
                    $("#Preguntas").append("<table width='100%'><tr><td><input type='checkbox' onclick='SetPregunta()' id='" + data[i].id_pregunta + "' name='" + data[i].id_pregunta + "' /> </td><td> " + data[i].contenido_pregunta + "</td></tr ></table > ");
                }
                SetPregunta();
            }
            else {
